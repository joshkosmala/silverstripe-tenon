...
class TenonProcessor extends Controller {
            ...
            if (strlen(trim($tenon_options["key"])) > 28 && ctype_xdigit(trim($tenon_options["key"]))) {
                ...
                if (!$this->existingPageHash()) {
                    if ($this->requestSend($tenon_options) && $this->responseSave() && $this->savePageHash()) {
                        ...
                        $this->log("TenonProcessor.analyse", "out=$out");
                        return $out;
                    }
                }
            }
    ...
    private function buildOptions($request){
        ...
        $this->log('TenonProcessor.buildOptions', 'source='.$config->TenonSource, SS_Log::NOTICE);
    ...
    private function existingPageHash(){
        ...
        $this->log("TenonProcessor.existingPageHash", "result=".print_r($this->hash_object, true));
    ...
    private function requestSend($tenon_options){
        $this->log("TenonProcessor.requestSend", "options=".print_r($tenon_options, true).", url=".$this->tenon_url);
        ...
        // Evaluate response
        $this->log("TenonProcessor.requestSend", "code=".$code." data=".print_r($data, true));
    ...
    private function responseSaveCount(){
        ...
        $this->log("TenonProcessor.responseSaveCount", "total=$total");
    ...
    private function responseSaveDeleteExisting(){
        ...
        $this->log("TenonProcessor.responseSaveDeleteExisting", "deleteCount=".$datalist->count());
    ...
    private function responseSaveErrorWarning(&$saved){
        ...
        $this->log("TenonProcessor.responseSaveErrorWarning", "count=" . count($this->tenon_response->resultSet));
    }
    ...
    private function responseSaveFailure(&$saved){
        ...
        $this->log("TenonProcessor.responseSaveFailure", "status=".$this->tenon_response->status.", out=$out");
        if (!$out){
            $result = new TenonResult();
            $timestamp = new SS_DateTime();
            $timestamp->setValue(date('Y-m-d H:i:s'));
    ...
    private function responseSaveScript(&$saved){
        $this->log("TenonProcessor.responseSaveScript", "count=".count($this->tenon_response->clientScriptErrors));
    ...
    private function responseSaveScriptCollate($stacktraces, $item){
        ...
        foreach($stacktraces as $trace){
            switch($item){
                ...
                default:
                    $atom = '';
                    $this->log("TenonProcessor.responseSaveScriptCollate", "Unexpected item: $item");
            }
            $out .= (($mult) ? "[$pos] " : "");
            $out .= $atom . "; ";
            $pos++;
        ...
        $this->log("TenonProcessor.responseSaveScriptCollate", "stacktraces=".count($stacktraces).", out=".$out);
        return $out;
    }
    ...
    private function savePageHash(){
        $this->log("TenonProcessor.savePageHash", "page=".$this->tenon_page.", hash=".$this->tenon_hash.", hashobjectset=".isset($this->hash_object));
